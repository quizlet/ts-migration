...
export function convertFlowType(path: NodePath<FlowType>): TSType {
  ...
  if (isNodePath(isGenericTypeAnnotation, path)) {
    ...
    if (id.name === '$Keys') {
      ...
      return tsTypeReference(
        identifier('Pick'),
        tsTypeParameterInstantiation([tsX, tsExclude])
      );
  ...
  if (isNodePath(isNullableTypeAnnotation, path)) {
    const tsT = convertFlowType(path.get('typeAnnotation') as NodePath<
      FlowType
    >);
    ...
    if (isFunctionDeclaration(pathStack[0].node)) {
      if (
        pathStack[1].node ===
        (pathStack[0].node as FunctionDeclaration).returnType
      ) {
        ...
        if (identifierPath.node.optional) {
          ...
          const argumentIndex = (pathStack[0]
            .node as FunctionDeclaration).params.indexOf(identifierPath.node);
  ...
  if (isNodePath(isNumberLiteralTypeAnnotation, path)) {
    return tsLiteralType(
      numericLiteral(
        (path as NodePath<NumberLiteralTypeAnnotation>).node.value!
      )
    );
  }
  ...
  if (isNodePath(isObjectTypeAnnotation, path)) {
      ...
      for (const [i, property] of objectTypeNode.properties.entries()) {
        if (isObjectTypeProperty(property)) {
          const tsPropSignature = tsPropertySignature(
            property.key,
            tsTypeAnnotation(
              convertFlowType(path.get(`properties.${i}`).get('value'))
            )
          );
        ...
        if (isObjectTypeSpreadProperty(property)) {
          // {p1:T, ...U} -> {p1:T} | U
          spreads.push(
            convertFlowType(path.get(`properties.${i}`).get('argument'))
          );
        }
      }
    }
    ...
    if (objectTypeNode.indexers && objectTypeNode.indexers.length > 0) {
      for (const [i, indexer] of objectTypeNode.indexers.entries()) {
        ...
        tsIndex.typeAnnotation = tsTypeAnnotation(
          convertFlowType(path.get(`indexers.${i}`).get('key') as NodePath<
            FlowType
          >)
        );
        const member = tsIndexSignature(
          [tsIndex],
          tsTypeAnnotation(
            convertFlowType(path.get(`indexers.${i}`).get('value') as NodePath<
              FlowType
            >)
          )
        );
    ...
    if (
      objectTypeNode.callProperties &&
      objectTypeNode.callProperties.length > 0
    ) {
  ...
  if (isNodePath(isUnionTypeAnnotation, path)) {
    ...
    return tsUnionType(
      flowTypes.map((_, i) =>
        convertFlowType(
          (path as NodePath<UnionTypeAnnotation>).get(`types.${i}`)
        )
      )
    );
  }
  ...
  if (isNodePath(isFunctionTypeAnnotation, path)) {
    ...
    const identifiers = path.node.params.map((p, i) => {
      ...
      const ftParam = nodePath.get(`params.${i}`) as NodePath<
        FunctionTypeParam
      >;
    ...
    const returnType = tsTypeAnnotation(
      convertFlowType(nodePath.get('returnType'))
    );
  ...
  if (isNodePath(isTupleTypeAnnotation, path)) {
    ...
    return tsTupleType(
      flowTypes.map((_, i) =>
        convertFlowType(
          (path as NodePath<TupleTypeAnnotation>).get(`types.${i}`)
        )
      )
    );
  }

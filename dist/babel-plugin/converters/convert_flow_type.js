...
function convertFlowType(path) {
    ...
    if (util_1.isNodePath(types_1.isNullableTypeAnnotation, path)) {
        ...
        while (types_1.isFlowType(pathStack[0].node) ||
            types_1.isTypeAnnotation(pathStack[0].node) ||
            types_1.isIdentifier(pathStack[0].node))
            pathStack.unshift(pathStack[0].parentPath);
        if (types_1.isFunctionDeclaration(pathStack[0].node)) {
            if (pathStack[1].node ===
                pathStack[0].node.returnType) {
                ...
                if (identifierPath.node.optional) {
                    ...
                    const argumentIndex = pathStack[0]
                        .node.params.indexOf(identifierPath.node);
                    if (pathStack[0].node.params
                        .slice(argumentIndex)
                        .every(node => node.optional)) {
                        // TODO:
